...
### 가설 및 기대결과
- 가설
    - 모델의 layer/head 수를 조정해 모델 용량을 낮추면, 학습 데이터의 세부 패턴을 과하게 외우는 현상을 줄이고 validation loss의 최저점을 더 낮출 수 있을 것이다.
    - 특히 Transformer layer 수를 줄이면 모델의 깊이와 파라미터 수가 감소하므로, 작은 NSMC 사전학습 데이터에서 과도하게 복잡한 패턴을 학습하는 것을 완화할 수 있을 것으로 예상하였다.
  
- 기대 결과
    - train_loss는 이전 실험보다 덜 빠르게 감소하거나, 같은 epoch 기준 더 높게 유지될 수 있다.
    - validation loss는 이전 실험보다 낮은 최저점에 도달할 가능성이 있다.
...
- 실험은 이전 실험보다 더 낮은 val_loss 구간에 도달하였다. 이전 실험에서는 val_loss가 약 5.0 근처에서 정체된 반면, layer/head를 줄인 실험에서는 약 4.95 수준까지 낮아졌다.
- layer 감소/head 증가는 전체적인 val_loss 최저점을 낮추는 데는 도움이 되었지만, 과적합 자체를 완전히 제거하지는 못했다. 
---
...
| 구현 | 구현 파일 | `src/train.py` |
| 실험 목적 | 변경 포인트 | 하이퍼 파라미터 조정 |
| 체크 포인트 | epoch 4 | best epoch 부터 시작 |
| 학습 | lr | 1e-4 -> 5e-5 -> 1e-5 |
| 최적화 | drop_rate | 0.25 |
| 최적화 | weight_decay | 0.025 |

### 6.5.2 결과

### 가설 및 기대결과
- 가설
    - best epoch 에서 학습률을 줄이면 더 작은 업데이트로 최적점 주변을 탐색할 수 있을 것이다.
