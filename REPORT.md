...
- train accuracy는 계속 상승하지만 validation accuracy는 초반 이후 하락해, 성능 개선보다는 과적합이 강하게 나타난 그래프이다.
...
| 구분 | 항목 | 값 |
| --- | --- | --- |
| **모델** | **drop_rate** | **0.1 -> 0.2** |
| 학습 | num_epochs | 30 -> 20 |
| 최적화 | learning_rate | 3e-4 -> 2e-4 |
| **최적화** | **weight_decay** | **0.02 -> 0.03** |
...
- train loss는 계속 감소하지만 validation loss는 초반 하락 후 다시 증가하므로, 중반 이후부터 과적합이 발생한 그래프라고 볼 수 있다.

#### 정확도 그래프

<img width="700" height="470" alt="image" src="https://github.com/user-attachments/assets/8d8e3d8e-5c22-4ca8-9684-f2ba2f3e4dcc" />
- train accuracy는 계속 상승하지만 validation accuracy는 중반 이후 거의 정체되므로, 과적합이 나타나긴 하지만 이전 그래프보다는 완만한 편이다.
...
- train loss는 계속 감소하지만 validation loss는 중반 이후 거의 정체되거나 약간 증가한다. 하지만 validation loss가 다시 증가하지 않는다.

#### 정확도 그래프

<img width="700" height="470" alt="image" src="https://github.com/user-attachments/assets/eb379b14-40e7-4582-b692-61c851e011f0" />

- train accuracy와 validation accuracy가 함께 높게 유지되지만, 중반 이후 train만 계속 상승해 성능은 좋아졌으나 약한 과적합이 나타난 그래프이다.
...
| **학습**  | **eval_freq**     | **50 -> 300**  |
| **학습**  | **eval_iter**     | **20 -> 10**   |
| **최적화** | **learning_rate** | **2e-4 -> 1e-4** |
| **최적화** | **weight_decay**  | **0.03 -> 0.05** |

### 6.4.2 epoch 별 loss
| epoch | step | train_loss | val_loss | train_acc | val_acc | val_ppl |
|---:|---:|---:|---:|---:|---:|---:|
| 1 | 3397 | 6.5716 | 6.5734 | 0.0859 | 0.0910 | 715.8278 |
| 2 | 6794 | 6.1763 | 6.1951 | 0.1104 | 0.1045 | 490.3242 |
| 3 | 10191 | 5.8728 | 5.9044 | 0.1268 | 0.1207 | 366.6459 |
| 4 | 13588 | 5.5631 | 5.7119 | 0.1352 | 0.1346 | 302.4514 |
| 5 | 16985 | 5.4597 | 5.5680 | 0.1570 | 0.1416 | 261.9059 |
| 6 | 20382 | 5.4352 | 5.4769 | 0.1518 | 0.1459 | 239.1088 |
| 7 | 23779 | 5.2383 | 5.3976 | 0.1486 | 0.1496 | 220.8663 |
| 8 | 27176 | 5.1949 | 5.3459 | 0.1535 | 0.1527 | 209.7429 |
| 9 | 30573 | 5.1615 | 5.2994 | 0.1641 | 0.1506 | 200.2238 |
| 10 | 33970 | 5.1273 | 5.2436 | 0.1717 | 0.1531 | 189.3566 |
...
| 항목 | 내용 |
| --- | --- |
| final train loss | 4.9923 |
| final validation loss | 5.2354 |
| best validation loss | 5.2436 |
| best epoch | 10 |
...
<img width="700" height="393" alt="image" src="https://github.com/user-attachments/assets/c10eb3b7-6dd0-4526-a129-0bda39c88f05" />
- train loss와 validation loss가 거의 함께 감소해 과적합은 많이 해소되었다.

#### 정확도 그래프

<img width="700" height="393" alt="image" src="https://github.com/user-attachments/assets/fbec38ab-d9e8-4c6c-8fba-f840f1f464f2" />
- train과 validation accuracy의 차이가 크지 않아 과적합은 어느 정도 해소되었다.

#### 생성 샘플

| epoch | 생성 샘플 |
| --- | --- |
| 1 | 영화 아있도 그걸로의 조을도 정말다.재밌다완전음아이.....한국는도 이로만아나고영화너무시기 하을 만들 |
| 2 | 영화 M도 영화...ㅡㅡ너무이런로이가 재밌습니다진짜아요.정말 너무 지성로 본것만 봤는데...이서 더욱해이 많이 자에 이런 일우의 성공 |
| 3 | 영화 ㅉㅉ정말 너무 재미있어요~그러난나..재밌음..아프게 보는 사람은 최고 ledveemovleend아아.이 |
| 4 | 영화 급가...정말 재밌는 영화너무너무 재미없지만 너무 좋다.스토리나여이런영화는 또한거라 말아쉬하던데진짜 좋았다너무 재미있어요.재미는 |
| 5 | 영화 ㄷㄷ재밌고 봤음. 시간낭비이다..아놔 재밌는 영화임...아..뭐냐영화였습니다이거 진짜 재미없는 영화김소설수없다.이 가르은 영화 |
| 6 | 영화 ^*평점의 삶을 만큼을 보는것같다는거리가 왜 봤었는데 재미없음진짜재밌다.이걸 영화라고 뭐냐기대안한마디로이걸 영화라고...오랜만에 잘 |
| 7 | 영화 inddfe kgrmaddaoriinre e laearye hione ere aturendomachi |
| 8 | 영화 +이렇게 유쾌하다.이러운영화이런 영화가 잘생겼나?지루할지 마티즌의 삶의 명품의 사랑을 뻤이영화!아~... |
| 9 | 영화 poedberyovicEBsinregtined y !영화라고 본건 처음 봤을때? 이정도만... 이런영화아내 |
| 10 | 영화 Gkkddimesto sfiececthantdceeov aninbecgoree tasthiat |
