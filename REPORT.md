...
- train loss와 validation loss가 거의 함께 감소해 과적합은 많이 해소됐지만, loss 자체가 아직 높은 편이라 성능은 더 개선할 여지가 있다.

#### 정확도 그래프

<img width="700" height="393" alt="image" src="https://github.com/user-attachments/assets/fbec38ab-d9e8-4c6c-8fba-f840f1f464f2" />
- train과 validation accuracy의 차이가 크지 않아 과적합은 어느 정도 해소됐지만, 전체 accuracy가 낮아서 성능 자체는 아직 부족한 그래프이다.
...
| ------- | ----------------- | -------- |
| **모델**  | **emb_dim**       | **128 -> 256**  |
| **모델**  | **drop_rate**     | **0.2 -> 0.15** |
| **학습**  | **num_epochs**    | **10 -> 12**   |
| **학습**  | **eval_freq**     | **300 -> 200**  |
| **최적화** | **learning_rate** | **1e-4 -> 2e-4** |
| **최적화** | **weight_decay**  | **0.05 -> 0.03** |
...
| 항목 | 내용 |
| --- | --- |
| final train loss | 4.0999 |
| final validation loss | 4.9311 |
| best validation loss | 4.9260 |
| best epoch | 11 |

#### 손실 그래프

<img width="691" height="393" alt="image" src="https://github.com/user-attachments/assets/eaaa8f78-0dce-48e3-a787-680aa24a1503" />

- train loss는 계속 낮아지지만 validation loss는 초반 이후 5.0 근처에서 정체된다. 성능은 회복됐지만 후반으로 갈수록 과적합이 생긴다.

#### 정확도 그래프

<img width="708" height="393" alt="image" src="https://github.com/user-attachments/assets/a707b44e-e794-4d24-ac60-9ac8c37913bb" />

- train accuracy와 validation accuracy가 모두 상승해 성능은 회복됐지만, 후반으로 갈수록 둘의 차이가 커져 약간의 과적합 경향도 함께 나타난 그래프이다.
...
| 12 | 영화보고나는데 이건 뭐라 말아놓고 또 다른거든지.......정말..이 영화보면서 마음도 있더니... 그게 너무나니, 애틈없는느낌..ㅠㅠ정말 재밌게 봤다..제 인생, |
